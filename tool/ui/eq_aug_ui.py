...
.kpi-pink      { background:#d14fb8; }
.kpi-purple    { background:#673ab7; }
.kpi-black     { background:#333333; }
...
def _load_inventory_text(text: str) -> Tuple[list[dict], list[dict]]:
    """
    Parse Raidloot /output inventory TSV.
    • Skips header row
    • Keeps **only** rows whose Name contains "(Aug"  ← filter added
    • Splits into equipped (ID>0) and unequipped (ID==0)
    """
    equipped, unequipped = [], []
    for line in text.splitlines():
        ...
        if item_id.lower() == "id":
            continue  # header
        if "(Aug" not in name:
            continue  # ignore non-augmentation items
